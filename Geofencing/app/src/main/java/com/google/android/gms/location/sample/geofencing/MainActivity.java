 ...
 * geofence.
 *
 * <p>This sample requires a device's Location settings to be turned on. It also requires the
...
public class MainActivity extends AppCompatActivity implements OnCompleteListener<Void> {
  ...
  /**
   * Removes geofences, which stops further notifications when the device enters or exits previously
  ...
  /**
   * Ensures that only one button is enabled at any time. The Add Geofences button is enabled if the
   * user hasn't yet added geofences. The Remove Geofences button is enabled if the user has added
   * geofences.
   */
